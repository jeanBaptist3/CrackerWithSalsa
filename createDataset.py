...
def generate_binary_strings(min_int, max_int):
    for i in range(min_int, max_int + 1):
        binary_string = struct.pack('I', i & 0xFFFF).hex()[:4]
        yield binary_string

min_int = -2**15 # Minimum 32-bit integer
max_int = 2**15 -1 # Maximum 32-bit integer
...
# Train the tokenizer
trainer = trainers.WordPieceTrainer(vocab_size= 2**16 + 37,show_progress = True,special_tokens=["[PAD]", "[CLS]", "[SEP]", "[MASK]", "[UNK]", "[NONC1]", "[NONC2]", "[NONC3]","[NONC0]"])
tokenizer.train_from_iterator(generate_binary_strings(min_int, max_int), trainer=trainer)


# Save the trained tokenizer to a file
tokenizer.save("tokenizer_stuff/custom_tokenizer_wordpiece.json")
